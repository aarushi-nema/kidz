...
LoginHandler = new function() {
    ...
    this.callback = function(response) {
        if (response.errCode == 0) {
            if (this.previous_url != null) {
                ...
                baseurl =   baseurl +"/ui/corporate_test_myarea";
                window.location =  baseurl;
            }
        } else {
        	if (response.message.indexOf("RESET_PASSWORD") != -1) {
                ...
                if (window.location.port != 80) {
		         ...
		         window.location =  baseurl+"/ui/confirm_invite?id="+tokens[0]+"&key="+field.value;
        	} else {
    ...
    this.call_ci_register = function() {
        ...
        var url = baseurl+"/c4t/corporate_test_company_register";
        var parameters= "";
        var resource = {};
