...
import application.kidz.defined.helper.SentenceTypeHelper;
import application.kidz.defined.helper.SightWordCategoryTypeHelper;
import application.kidz.defined.resource.SentenceType;
...
public class SentenceTypeServlet extends AmrSingleColumnDBApplicationServlet {
	/**
	 * 
	 */
	...
	public BaseHTMLComponent getListView(UIServletContext context) {
		...
		BaseResource[] resources = SentenceTypeHelper.getInstance().getAll(new String[]{"name"});
		TR tr = new TR();
		int index = 0;
		for(BaseResource r : resources) {
			SentenceType type = (SentenceType) r;
			TD td = new TD();
			td.addStyle("text-align","center");
			H2 h2 = new H2("");
			A a = new A();
			a.setHref("/sentence_category?type="+type.getId());
