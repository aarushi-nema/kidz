...
public class SentenceCategoryServlet extends AmrSingleColumnDBApplicationServlet {
	/**
	 * 
	 */
	...
	public BaseHTMLComponent getListView(UIServletContext context) {
		...
		BaseResource[] resources = SentenceCategoryHelper.getInstance().getByExpression(e, new String[]{"name"});
		TR tr = new TR();
		int index = 0;
		for(BaseResource r : resources) {
			SentenceCategory category = (SentenceCategory) r;
			TD td = new TD();
			td.addAttribute("align","center");
			td.addStyle("text-align","center");
			H2 h2 = new H2("");
			A a = new A();
			a.setHref("/sentence?category="+category.getId());
