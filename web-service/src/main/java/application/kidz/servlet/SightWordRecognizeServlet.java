...
public class SightWordRecognizeServlet extends AmrSingleColumnDBApplicationServlet {
	/**
	 * 
	 */
	...
	public BaseHTMLComponent getListView(UIServletContext context) {
		...
		BaseResource[] resources = SightWordHelper.getInstance().getAll();
		int index = (int)new Date().getTime()%resources.length;
		SightWord word = (SightWord) resources[index];
		Div wordDiv = new Div();
		div.addChild(new BR());
		div.addChild(new BR());
		div.addChild(new BR());
