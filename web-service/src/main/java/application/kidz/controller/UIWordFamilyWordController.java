...
import application.kidz.servlet.WordFamilyWordServlet;
...
@Controller
public class UIWordFamilyWordController {
    @Autowired
    private HttpServletRequest request;

    @RequestMapping(value ="/word_family_word",method = RequestMethod.GET)
    public @ResponseBody String doGet(@CookieValue(value="session_id",required=false) String sessionId,
        ...
        try {
            ...
            if (!Util.isEmpty(family)) {
                context.addParam("family",family);
            }

            context.setDomainName(request.getServerName());
            WordFamilyWordServlet servlet = new WordFamilyWordServlet();
